...
### Not all site-years are present in the number fledged dataset (conditional on success) 
## this model estimates the missing site, year and site-year effects for the number fledged fitness function parameters
...
write("functions {
      ...
      int<lower=0> N_site;     // number of sites (same for bird bernoulli)
      int<lower=0> N_year;     // number of years (same for bird bernoulli)
      int<lower=0> N_siteyear;     // number of site-year combinations (same for bird bernoulli)
      ...
      int<lower=0> N_bp;     // length of truncated bird data, 0 fledge nests removed
      ...
      // For estimating missing levels in bp
      int<lower=0> N_siteyear_missing;     // missing site-years
      int<lower=0> bb_missing_in_bp[N_siteyear_missing];      // which bb levels are missing in bp
      }
      ...
      vector[2] bbp_reg;     // one coefficient for each gaussian function parameter
      ...
      real lambda;     // dispersion parameter
      
      matrix[N_siteyear_missing,3] siteyear_missing_effs;     //estimated bp site-year effects for levels with no bp data
      ...
      matrix[N_siteyear_missing,3] siteyear_missing_p;     // equivalent for bp estimates with no site-year data
      ...
      siteyear_missing_p[,{1,3}] = diag_post_multiply(siteyear_effs_bb[bb_missing_in_bp,{1,3}],bbp_reg);     // equivalent for bp estimates with no site-year data
      siteyear_missing_p[,2] = siteyear_effs_bb[bb_missing_in_bp,2];
      
      }
      
      model{
      // Caterpillar
      vector[N_c] y_1c;     // logscale cater estimates for mean of normal dist
      ...
      // MISSING LEVELS IN BP
      siteyear_missing_effs[,1] ~ normal(siteyear_missing_p[,1],sd_siteyear_bp[1]);
      siteyear_missing_effs[,2] ~ normal(siteyear_missing_p[,2],sd_siteyear_bp[2]);
      siteyear_missing_effs[,3] ~ normal(siteyear_missing_p[,3],sd_siteyear_bp[3]);
      ...
      "FullMMH_estmissing.stan"
)
stanc("FullMMH_estmissing.stan")
FullMMH_estmissing_stan <- stan_model("FullMMH_estmissing.stan")

data_FullMMH_estmissing <- list(
  N_c=nrow(cater),
  ...
  N_siteyear_missing=length(setdiff(SY_id_p$siteyear_id,SY_id_p$siteyear_id_c)),
  bb_missing_in_bp=as.numeric(setdiff(SY_id_p$siteyear_id,SY_id_p$siteyear_id_c))
)
...
FullMMH_estmissing_mod <- sampling(object=FullMMH_estmissing_stan, data=data_FullMMH_estmissing,
                                           chains=4, cores=4, warmup= 2000, iter=24000, thin=20,
                                           init=init, control = list(adapt_delta = 0.94,stepsize = 0.1, max_treedepth = 12)) 
saveRDS(FullMMH_estmissing_mod,"FullMMH_estmissing_mod.rds") 

mod <- FullMMH_estmissing_mod

rstan::traceplot(mod, pars=c("mu", "logsigma", "logmax",
                             "theta_bb", "logWmax_bb", "logomega_bb",
                             "theta_bp", "logWmax_bp", "logomega_bp",
